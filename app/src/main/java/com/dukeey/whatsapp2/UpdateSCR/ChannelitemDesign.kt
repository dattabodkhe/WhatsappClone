...
@Composable
fun ChannelitemDesgin(channel : Channel){
	...
	Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp, vertical = 8.dp),
		...
		Image(painter = painterResource(id = channel.image), contentDescription = null,
			modifier = Modifier.size(60.dp).padding(4.dp)
		)
		Spacer(modifier = Modifier.width(12.dp))
		
		Column {
			Text(text = channel.name, fontSize = 21.sp, fontWeight = FontWeight.Bold)
			
			Text(text = channel.descrition, color = Color.Gray,
				fontSize = 14.sp
			)
		}
		...
		Button(onClick = {isFollowing=!isFollowing}, colors = ButtonDefaults.buttonColors(
			...
			Text(text = if (isFollowing){"following"}else{
				"folow"
